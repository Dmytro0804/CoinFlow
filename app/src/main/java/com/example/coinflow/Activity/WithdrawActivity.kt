...
import com.example.coinflow.databinding.ActivityWithdrawBinding
...
class WithdrawActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val currentBalanceUah = prefs.getUahBalance()
        binding.availableBalanceTxt.text = "Доступно: ${String.format(Locale.US, "%,.2f", currentBalanceUah)} ₴"
        ...
        if (savedCardNumber.isNotEmpty()) {
            binding.cardNumberEdt.setText(savedCardNumber)
        }
        ...
        binding.sendWithdrawBtn.setOnClickListener {
            val cardNumberStr = binding.cardNumberEdt.text.toString()
            val cardNumberClean = cardNumberStr.replace(" ", "")
            val amountStr = binding.amountEdt.text.toString().trim().replace(",", "")
            val amount = amountStr.toDoubleOrNull()

            if (cardNumberClean.length < 16) { Toast.makeText(this, "Введіть коректний номер картки", Toast.LENGTH_SHORT).show(); return@setOnClickListener }
            if (amount == null || amount <= 0) { Toast.makeText(this, "Введіть коректну суму", Toast.LENGTH_SHORT).show(); return@setOnClickListener }
            if (amount > currentBalanceUah) { Toast.makeText(this, "Недостатньо коштів на балансі!", Toast.LENGTH_LONG).show(); return@setOnClickListener }

            showPasswordDialog("Підтвердження", "Введіть пароль для виведення коштів.") {
                prefs.addUahBalance(-amount)
                ...
                showReceiptDialog(
                    ...
                    finalStr = "${String.format(Locale.US, "%,.2f", totalToCard)} ₴",
